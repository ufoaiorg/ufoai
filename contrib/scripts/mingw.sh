...
mkdir ${mingwtmp} 2> /dev/null

if [ -d "./${mingwtmp}" ]; then
	echo "Start"
else
	echo "Could not create directory ${mingwtmp}"
	exit 1;
fi
...
any2deb mingw32-zlib1-dev ${version} zlib-${version}-lib.zip ${mingwprefix} >> /dev/null
...
any2deb mingw32-libpng12-dev ${version} libpng-${version}-lib.zip ${mingwprefix} >> /dev/null
if [ "${dont_install}" -ne "1" ]
then
...
any2deb mingw32-libjpeg-dev ${version} jpeg-${version}-4-lib.zip ${mingwprefix} >> /dev/null
if [ "${dont_install}" -ne "1" ]
then
...
any2deb mingw32-libiconv-dev ${version} libiconv-${version}-1-lib.zip ${mingwprefix} >> /dev/null
if [ "${dont_install}" -ne "1" ]
then
...
any2deb mingw32-libintl-dev ${version} libintl-${version}-lib.zip ${mingwprefix} >> /dev/null
if [ "${dont_install}" -ne "1" ]
then
...
any2deb mingw32-freetype-dev ${version} freetype-${version}-lib.zip ${mingwprefix} >> /dev/null
if [ "${dont_install}" -ne "1" ]
then
...
mkdir -p build-sdl${mingwprefix}

dir=`pwd`/build-sdl${mingwprefix}/
cd SDL-${version}
cp -rv bin include lib share $dir/
sed "s|^prefix=.*|prefix=${mingwprefix}|" < bin/sdl-config > $dir/bin/sdl-config;
chmod 755 $dir/bin/sdl-config;
sed "s|^libdir=.*|libdir=\'${mingwprefix}/lib\'|" <lib/libSDL.la > $dir/lib/libSDL.la; \
cd ..
...
any2deb mingw32-libsdl-ttf2.0-dev ${version} SDL_ttf-${version}-1-i386-mingw32.tar.gz ${mingwprefix} >> /dev/null
if [ "${dont_install}" -ne "1" ]
then
...
any2deb mingw32-libsdl-mixer-dev ${version} SDL_mixer-${version}-1-i386-mingw32.tar.gz ${mingwprefix} >> /dev/null
if [ "${dont_install}" -ne "1" ]
then
...
cd ..
any2deb mingw32-libcurl-dev ${version} libcurl-${version}-win32-nossl.tar.gz ${mingwprefix} >> /dev/null
if [ "${dont_install}" -ne "1" ]
then
