...
extract_archive_gz() 
{
	file=${1}
	target=${2}
	${TAR} -xzf "${DOWNLOAD_DIR}/${file}" -C "${target}" >> ${LOGFILE_NAME} 2>&1
...
extract_archive_bz2() 
{
	file=${1}
	target=${2}
	${TAR} -xjf "${DOWNLOAD_DIR}/${file}" -C "${target}" >> ${LOGFILE_NAME} 2>&1
	check_error "Failed to extract ${file}"
}
...
#	download_archive http://downloads.xiph.org/releases/ogg/ libvorbis-1.2.0.tar.gz libvorbis.tar.gz
	...
	download_archive http://download.berlios.de/codeblocks/ CB_20090413_rev5535_win32.7z codeblocks.7z
	...
	download_archive http://mattn.ninex.info/download gtkglext-1.2.tar.bz2 gtkglext-dev.tar.bz2
