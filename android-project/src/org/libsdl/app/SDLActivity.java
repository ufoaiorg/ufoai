...
public class SDLActivity extends Activity {
	...
	public static void deleteGLContext() {
		if (SDLActivity.mEGLDisplay != null
			...
			egl.eglDestroyContext(SDLActivity.mEGLDisplay,
					SDLActivity.mEGLContext);
			SDLActivity.mEGLContext = EGL10.EGL_NO_CONTEXT;

			if (SDLActivity.mEGLSurface != EGL10.EGL_NO_SURFACE) {
				egl.eglDestroySurface(SDLActivity.mEGLDisplay,
	...
	public static boolean initEGL(int majorVersion, int minorVersion,
		...
		try {
			...
			if (SDLActivity.mEGLDisplay != null
				...
				if (!egl.eglChooseConfig(SDLActivity.mEGLDisplay, attribs,
						configs, 1, num_config) || num_config[0] == 0) {
				...
				// eglChooseConfig returns a number of configurations that match
				// or exceed the requested attribs.
				// From those, we select the one that matches our requirements
				// more closely
				Log.v("SDL", "Got " + num_config[0] + " valid modes from egl");
				for (int i = 0; i < num_config[0]; i++) {
					bitdiff = 0;
					// Go through some of the attributes and compute the bit
					// difference between what we want and what we get.
					for (int j = 0;; j += 2) {
						...
						if (attribs[j + 1] != EGL10.EGL_DONT_CARE
								&& (attribs[j] == EGL10.EGL_RED_SIZE
										...
										|| attribs[j] == EGL10.EGL_DEPTH_SIZE || attribs[j] == EGL10.EGL_STENCIL_SIZE)) {
							egl.eglGetConfigAttrib(SDLActivity.mEGLDisplay,
									configs[i], attribs[j], value);
							bitdiff += value[0] - attribs[j + 1]; // value is
																	// always >=
																	// attrib
						}
					}
				...
				Log.d("SDL", "Selected mode with a total bit difference of "
						+ bestdiff);
	...
	public static boolean createEGLSurface() {
		if (SDLActivity.mEGLDisplay != null && SDLActivity.mEGLConfig != null) {
			...
			if (SDLActivity.mEGLSurface == EGL10.EGL_NO_SURFACE) {
				...
				SDLActivity.mEGLSurface = egl.eglCreateWindowSurface(
			...
			if (egl.eglGetCurrentContext() != SDLActivity.mEGLContext) {
				if (!egl.eglMakeCurrent(SDLActivity.mEGLDisplay,
						SDLActivity.mEGLSurface, SDLActivity.mEGLSurface,
						SDLActivity.mEGLContext)) {
					Log.e("SDL",
			...
			Log.e("SDL", "Surface creation failed, display = "
					+ SDLActivity.mEGLDisplay + ", config = "
	...
	public static int audioInit(int sampleRate, boolean is16Bit,
		...
		// Instantiating AudioTrack can "succeed" without an exception and the
		// track may still be invalid
		// Ref:
...
class SDLSurface extends SurfaceView implements SurfaceHolder.Callback,
	...
	public void surfaceDestroyed(SurfaceHolder holder) {
		 ...
		 * Otherwise there's BAD_NATIVE_WINDOW errors coming from
		 * eglCreateWindowSurface on resume Ref:
		 * http://stackoverflow.com/questions
		 * /8762589/eglcreatewindowsurface-on-ics-and-switching-from-2d-to-3d
		 */
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int width,
		...
		if (SDLActivity.mSDLThread == null) {
			...
			// Multiple sequential calls to surfaceChanged are handled
			// internally by handleResume
