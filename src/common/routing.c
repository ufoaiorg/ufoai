 ...
 */
static trace_t RT_ObstructedTrace (RT_data_t *rtd, const vec3_t start, const vec3_t end, actorSizeEnum_t actorSize, int hi, int lo)
{
 ...
 */
static int RT_FindOpeningFloorFrac (RT_data_t *rtd, const vec3_t start, const vec3_t end, const actorSizeEnum_t actorSize, const float frac, const int startingHeight)
{
 ...
 */
static int RT_FindOpeningCeilingFrac (RT_data_t *rtd, const vec3_t start, const vec3_t end, const actorSizeEnum_t actorSize, const float frac, const int startingHeight)
{
 ...
 * @return The absolute height of the found floor in QUANT units.
 */
static int RT_FindOpeningFloor (RT_data_t *rtd, const vec3_t start, const vec3_t end, const actorSizeEnum_t actorSize, const int startingHeight, const int floorLimit)
{
	...
	if (start[0] == end[0] || start[1] == end[1]) {
		...
		midfloor = RT_FindOpeningFloorFrac(rtd, start, end, actorSize, 0.5, startingHeight);
		if (debugTrace)
		...
		/* 1/3 point */
		midfloor = RT_FindOpeningFloorFrac(rtd, start, end, actorSize, 0.33, startingHeight);
		if (debugTrace)
			Com_Printf("1/3floor:%i.\n", midfloor);

		/* 2/3 point */
		midfloor2 = RT_FindOpeningFloorFrac(rtd, start, end, actorSize, 0.66, startingHeight);
		if (debugTrace)
 ...
 * @return The absolute height of the found ceiling in QUANT units.
 */
static int RT_FindOpeningCeiling (RT_data_t *rtd, const vec3_t start, const vec3_t end, const actorSizeEnum_t actorSize, const int startingHeight, const int ceilLimit)
{
	int midceil;

	if (start[0] == end[0] || start[1] == end[1]) {
		/* For orthogonal dirs, find the height at the midpoint. */
		midceil = RT_FindOpeningCeilingFrac(rtd, start, end, actorSize, 0.5, startingHeight);
		if (debugTrace)
		...
		/* If this is diagonal, trace the 1/3 and 2/3 points instead. */
		/* 1/3 point */
		midceil = RT_FindOpeningCeilingFrac(rtd, start, end, actorSize, 0.33, startingHeight);
		if (debugTrace)
			Com_Printf("1/3ceil:%i.\n", midceil);

		/* 2/3 point */
		midceil2 = RT_FindOpeningCeilingFrac(rtd, start, end, actorSize, 0.66, startingHeight);
		if (debugTrace)
 ...
 */
static int RT_TraceOpening (RT_data_t *rtd, const routing_t * map, const actorSizeEnum_t actorSize, const vec3_t start, const vec3_t end, const int ax, const int ay, const int bottom, const int top, int lo, int hi, int *lo_val, int *hi_val)
{
	trace_t tr = RT_ObstructedTrace(rtd, start, end, actorSize, hi, lo);
	if (tr.fraction >= 1.0) {
		lo = RT_FindOpeningFloor(rtd, start, end, actorSize, lo, bottom);
		hi = RT_FindOpeningCeiling(rtd, start, end, actorSize, hi, top);
 ...
 * @return The new z value of the actor after traveling in this direction from the starting location.
 */
static int RT_FindOpening (RT_data_t *rtd, const routing_t * map, const actorSizeEnum_t actorSize, place_t* from, const int ax, const int ay, const int bottom, const int top, int *lo_val, int *hi_val)
{
	vec3_t start, end;
	pos3_t pos;
	int temp_z;
	...
	if (top - bottom < PATHFINDING_MIN_OPENING * 2) {
		...
		temp_z = RT_TraceOpening(rtd, map, actorSize, start, end, ax, ay, hifloor, top, lo, hi, lo_val, hi_val);
	} else {
		...
		while (lo <= top - PATHFINDING_MIN_OPENING) {
			...
			temp_z = RT_TraceOpening(rtd, map, actorSize, start, end, ax, ay, bottom, top, lo, lo + 1, lo_val, hi_val);
			if (temp_z != RT_NO_OPENING)
				break;
...
*/
static int RT_MicroTrace (RT_data_t *rtd, const routing_t * map, const actorSizeEnum_t actorSize, place_t* from, const int ax, const int ay, const int az, const int stairwaySituation, opening_t* opening)
{
 ...
 */
static int RT_TraceOnePassage (RT_data_t *rtd, const routing_t * map, const actorSizeEnum_t actorSize, place_t* from, place_t* to, opening_t* opening)
{
	...
	az = RT_FindOpening(rtd, map, actorSize, from, ax, ay, lower, upper, &opening->base, &hi);
	...
	if (az != RT_NO_OPENING && opening->size >= PATHFINDING_MIN_OPENING - PATHFINDING_MIN_STEPUP) {
		...
		if (opening->size < CELL_HEIGHT
			...
			const int bonusSize = RT_MicroTrace(rtd, map, actorSize, from, ax, ay, az, stairway, opening);
...
static void RT_TracePassage (RT_data_t *rtd, const int x, const int y, const int z, const int ax, const int ay, opening_t* opening)
{
	...
	RT_TraceOnePassage(rtd, rtd->map, rtd->actorSize, &from, placeToCheck, opening);
	if (opening->size < PATHFINDING_MIN_OPENING) {
		if (debugTrace)
			Com_Printf(" No opening found.\n");
		/* If we got here, then there is no opening from floor to ceiling. */
 ...
 */
static int RT_UpdateConnection (RT_data_t *rtd, const actorSizeEnum_t actorSize, const int x, const int y, const int ax, const int ay, const int z, const int dir)
{
...
void RT_UpdateConnectionColumn (mapTiles_t *mapTiles, routing_t * map, const int actorSize, const int x, const int y, const int dir, const char **list)
{
	...
	for (z = 0; z < PATHFINDING_HEIGHT; z++) {
		new_z = RT_UpdateConnection(&rtd, actorSize, x, y, ax, ay, z, dir);
		assert(new_z >= z);
		z = new_z;
	}
}
