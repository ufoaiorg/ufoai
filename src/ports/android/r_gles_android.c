...
#define CONVERT_PIXEL_RGBA8888_RGB565( X ) CONVERT_PIXEL_RGB888_RGB565( ( ( (X) & 0xff ) ), ( ((X) & 0xff00) / 0x100 ), ( ((X) & 0xff0000) / 0x10000 ) )
#define CONVERT_PIXEL_RGBA8888_BGR565( X ) CONVERT_PIXEL_BGR888_RGB565( ( ( (X) & 0xff ) ), ( ((X) & 0xff00) / 0x100 ), ( ((X) & 0xff0000) / 0x10000 ) )
...
void R_TextureConvertNativePixelFormat(void * pixels, int w, int h, int alphaUsed)
{
	if(alphaUsed)
	{
		...
		for(int x = 0; x < w; x++)
		{
			...
			out[y*w+x] = CONVERT_PIXEL_ABGR8888_RGBA4444(in[y*w+x]);
#else
			out[y*w+x] = CONVERT_PIXEL_ABGR8888_RGBA5551(in[y*w+x]);
#endif
		}
	}
	else
	{
		...
		for(int x = 0; x < w; x++)
		{
			out[y*w+x] = CONVERT_PIXEL_RGBA8888_BGR565(in[y*w+x]);
		}
		*/
	}
}
