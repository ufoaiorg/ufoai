...
{
	public:
		static void setReplaceStr (const std::string& name);
		static void setFindStr (const std::string& name);
		static bool isOpen ();
		static void show ();
		typedef FreeCaller<&FindTextureDialog::show> ShowCaller;
		static void updateTextures (const std::string& name);
...
GtkWindow* FindTextureDialog::BuildDialog ()
{
	...
	gtk_table_attach(GTK_TABLE (table), entry, 1, 2, 0, 1, (GtkAttachOptions) (GTK_EXPAND | GTK_FILL),
	...
	entry = gtk_entry_new();
	gtk_widget_show(entry);
	gtk_table_attach(GTK_TABLE (table), entry, 1, 2, 1, 2, (GtkAttachOptions) (GTK_EXPAND | GTK_FILL),
			(GtkAttachOptions) (0), 0, 0);
	g_signal_connect(G_OBJECT(entry), "focus_in_event",
...
void FindTextureDialog::updateTextures (const std::string& name)
{
	if (isOpen()) {
		...
		if (g_bFindActive) {
			...
			setReplaceStr(name.substr(length));
		}
	}
}
...
void FindTextureDialog::setFindStr (const std::string& name)
{
	g_FindTextureDialog.exportData();
...
void FindTextureDialog::setReplaceStr (const std::string& name)
{
	g_FindTextureDialog.exportData();
...
void FindTextureDialog_selectTexture (const std::string& name)
{
