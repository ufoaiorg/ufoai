...
static void toolbar_insert (GtkToolbar *toolbar, const char* icon, const char* text, const char* tooltip,
...
static void PlugInToolbar_AddButton (GtkToolbar* toolbar, const IToolbarButton* button)
{
	toolbar_insert(toolbar, button->getImage(), button->getText(), button->getTooltip(), button->getType(),
			GTK_SIGNAL_FUNC(ActivateToolbarButton), reinterpret_cast<gpointer> (const_cast<IToolbarButton*> (button)));
}

static GtkToolbar* g_plugin_toolbar = 0;

static void PluginToolbar_populate ()
{
	class AddToolbarItemVisitor: public ToolbarModules::Visitor
	{
		...
		public:
			...
			void visit (const std::string& name, const _QERPlugToolbarTable& table) const
			{
