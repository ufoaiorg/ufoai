...
void RadiantWindowObserver::onMouseDown (const WindowVector& position, GdkEventButton* event)
{
	...
	if (observerEvent == ui::obsManipulate || observerEvent == ui::obsSelect || observerEvent == ui::obsToggle
			|| observerEvent == ui::obsToggleFace || observerEvent == ui::obsReplace || observerEvent
			== ui::obsReplaceFace) {
		_mouseDown = true;
 ...
 * The argument state is unused at the moment.
 */
...
void RadiantWindowObserver::onMouseUp (const WindowVector& position, GdkEventButton* event)
{
	...
	bool reactToEvent = (observerEvent == ui::obsManipulate || observerEvent == ui::obsSelect || observerEvent
			== ui::obsToggle || observerEvent == ui::obsToggleFace || observerEvent == ui::obsReplace || observerEvent
			== ui::obsReplaceFace);
