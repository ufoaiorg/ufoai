...
class OpenGLShaderCache: public ShaderCache, public TexturesCacheObserver, public ModuleObserver, public render::OpenGLStateManager {
		...
		typedef HashedCache<std::string, OpenGLShader, HashString, std::equal_to<std::string>,
				CreateOpenGLShader> Shaders;
		Shaders m_shaders;
	...
	public:
		...
		void render(RenderStateFlags globalstate, const Matrix4& modelview,
			...
			glPolygonOffset(-1, 1);
			{
				const GLubyte pattern[132] = { 0xAA, 0xAA, 0xAA, 0xAA, 0x55, 0x55, 0x55, 0x55,
