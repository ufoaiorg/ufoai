...
public:
	...
	XMLRegistry() :
		...
		_origXmlDocPtr->children = xmlNewDocNode(_origXmlDocPtr, NULL,
				xmlCharStrdup(_topLevelNode.c_str()), xmlCharStrdup(""));
	...
	std::string prepareKey(const std::string& key) {
		...
		if (returnValue.length() == 0) {
			...
			returnValue = std::string("/") + _topLevelNode + std::string("/")
					+ key;
			return returnValue;
		}
	}
	 ...
	 */
	xml::Node createKeyWithName(const std::string& path,
		...
		xmlNodePtr createdNode = xmlNewChild(insertPoint, NULL, xmlCharStrdup(
				key.c_str()), xmlCharStrdup(""));
		...
		if (createdNode != NULL) {
			...
			globalOutputStream()
					<< "XMLRegistry: Critical: Could not create insert point.\n";
			return NULL;
		}
	}
		...
		if (parts.size() > 0) {
			...
			for (unsigned int i = 0; i < parts.size(); i++) {
				...
				if (nodeList.size() > 0) {
					...
					createdNode = xmlNewChild(insertPoint, NULL, xmlCharStrdup(
							parts[i].c_str()), xmlCharStrdup(""));
					//addWhiteSpace(createdNode);
			...
			globalOutputStream()
					<< "XMLRegistry: Cannot insert key/path without slashes.\n";
			return NULL;
		}
	}
	...
	void set(const std::string& key, const std::string& value) {
		...
		if (nodeList.size() > 0) {
			...
			globalOutputStream() << "XMLRegistry: Critical: Key "
					<< fullKey.c_str()
	...
	void importFromFile(const std::string& importFilePath,
		...
		if (importNodeList.size() > 0) {
			...
			globalOutputStream()
					<< "XMLRegistry: Critical: ImportNode could not be found.\n";
		}

		globalOutputStream() << "XMLRegistry: Importing XML file: "
				<< importFilePath.c_str() << "\n";
		...
		if (pImportDoc) {
			...
			if (importNode->children != NULL) {
				...
				globalOutputStream()
						<< "XMLRegistry: Critical: Could not import XML file. importNode is NULL!\n";
			}
		} else {
			globalOutputStream() << "XMLRegistry: Critical: Could not parse "
					<< importFilePath.c_str() << "\n";
			globalOutputStream()
	...
	void exportToFile(const std::string& key, const std::string& filename) {
		...
		if (result.size() > 0) {
			...
			globalOutputStream() << "XMLRegistry: Saved " << key << " to "
					<< filename << "\n";
		} else {
			globalOutputStream() << "XMLRegistry: Failed to save path "
					<< fullKey << "\n";
		}
	}

	// Add an observer watching the <observedKey> to the internal list of observers.
	void addKeyObserver(RegistryKeyObserver* observer,
			const std::string& observedKey) {
...
private:
	...
	void notifyKeyObservers(const std::string& changedKey) {
		for (KeyObserverMap::iterator i = _keyObservers.begin(); i
				!= _keyObservers.end(); i++) {
...
StaticRegisterModule staticRegisterXMLRegistrySystem(
		StaticXMLRegistrySystemModule::instance());
