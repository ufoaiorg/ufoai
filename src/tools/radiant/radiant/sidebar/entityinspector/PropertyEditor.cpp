...
PropertyEditor::PropertyEditor (Entity* ent, const std::string& key, const std::string& type) :
	_widget(gtk_vbox_new(FALSE, 0)), _entity(ent), _key(key), _type(type), _applyButtonHbox(NULL), _editWindow(NULL)
{
...
GtkWidget* PropertyEditor::getEditWindow ()
{
	if (_editWindow == NULL) {
		...
		gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(_editWindow), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
	}
	return _editWindow;
}
...
void PropertyEditor::callbackApply (GtkWidget* caller, PropertyEditor* self)
{
	...
	std::string cmd = "entitySetKeyValue -key \"" + self->_key + "\" -value \"" + newValue + "\"";
