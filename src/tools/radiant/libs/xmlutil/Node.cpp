...
NodeList Node::getChildren() const {
	...
	for (xmlNodePtr child = _xmlNode->children; child != NULL; child
			= child->next) {
...
NodeList Node::getNamedChildren(const std::string& name) const {
	...
	for (xmlNodePtr child = _xmlNode->children; child != NULL; child
		...
		if (xmlStrcmp(child->name,
				reinterpret_cast<const xmlChar*> (name.c_str())) == 0) {
			retval.push_back(child);
		}
	}
...
void Node::setAttributeValue(const std::string& key, const std::string& value) {
	xmlSetProp(_xmlNode, xmlCharStrdup(key.c_str()), xmlCharStrdup(
			value.c_str()));
}
...
std::string Node::getAttributeValue(const std::string& key) const {
	...
	for (xmlAttrPtr attr = _xmlNode->properties; attr != NULL; attr
			= attr->next) {
		if (xmlStrcmp(attr->name,
				reinterpret_cast<const xmlChar*> (key.c_str())) == 0) {
...
std::string Node::getContent() const {
	if (_xmlNode->children && _xmlNode->children->content) {
		return std::string(
				reinterpret_cast<const char*> (_xmlNode->children->content));
	} else {
		return "";
	}
}
