...
inline Plane3 plane3_normalised(const Plane3& plane) {
	...
	return Plane3(plane.a * rmagnitude, plane.b * rmagnitude, plane.c * rmagnitude, plane.d
			* rmagnitude);
}
...
inline Plane3 plane3_translated(const Plane3& plane, const Vector3& translation) {
	...
	transformed.d = -((-plane.d * transformed.a + translation.x()) * transformed.a + (-plane.d
			* transformed.b + translation.y()) * transformed.b + (-plane.d * transformed.c
			+ translation.z()) * transformed.c);
	return transformed;
}
...
inline Plane3 plane3_transformed(const Plane3& plane, const Matrix4& transform) {
	...
	transformed.d = -((-plane.d * transformed.a + transform[12]) * transformed.a + (-plane.d
			* transformed.b + transform[13]) * transformed.b + (-plane.d * transformed.c
			+ transform[14]) * transformed.c);
	return transformed;
}

inline Plane3 plane3_inverse_transformed(const Plane3& plane, const Matrix4& transform) {
	return Plane3(transform[0] * plane.a + transform[1] * plane.b + transform[2] * plane.c
			+ transform[3] * plane.d, transform[4] * plane.a + transform[5] * plane.b
			+ transform[6] * plane.c + transform[7] * plane.d, transform[8] * plane.a
			+ transform[9] * plane.b + transform[10] * plane.c + transform[11] * plane.d,
			transform[12] * plane.a + transform[13] * plane.b + transform[14] * plane.c
					+ transform[15] * plane.d);
}
