...
inline Plane3 plane3_normalised (const Plane3& plane)
{
	...
	return Plane3(plane.a * rmagnitude, plane.b * rmagnitude, plane.c * rmagnitude, plane.d * rmagnitude);
}
...
inline Plane3 plane3_translated (const Plane3& plane, const Vector3& translation)
{
	...
	transformed.d = -((-plane.d * transformed.a + translation.x()) * transformed.a + (-plane.d * transformed.b
			+ translation.y()) * transformed.b + (-plane.d * transformed.c + translation.z()) * transformed.c);
	return transformed;
}
...
inline Plane3 plane3_transformed (const Plane3& plane, const Matrix4& transform)
{
	...
	transformed.d = -((-plane.d * transformed.a + transform[12]) * transformed.a + (-plane.d * transformed.b
			+ transform[13]) * transformed.b + (-plane.d * transformed.c + transform[14]) * transformed.c);
	return transformed;
}

inline Plane3 plane3_inverse_transformed (const Plane3& plane, const Matrix4& transform)
{
	return Plane3(transform[0] * plane.a + transform[1] * plane.b + transform[2] * plane.c + transform[3] * plane.d,
...
{
	return vector3_equal_epsilon(self.normal(), other.normal(), c_PLANE_NORMAL_EPSILON) && float_equal_epsilon(
			self.dist(), other.dist(), c_PLANE_DIST_EPSILON);
}
...
template<typename Element>
inline Plane3 plane3_for_points (const BasicVector3<Element>& p0, const BasicVector3<Element>& p1, const BasicVector3<
		Element>& p2)
{
	Plane3 self;
