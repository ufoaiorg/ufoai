...
class NonModalEntry
{
	...
	public:
		NonModalEntry (const Callback& apply, const Callback& cancel) :
			m_editing(false), m_apply(apply), m_cancel(cancel)
		{
		}
...
class NonModalSpinner
{
	...
	public:
		...
		void connect (GtkSpinButton* spin)
		{
			guint
					handler =
							g_signal_connect(G_OBJECT(gtk_spin_button_get_adjustment(spin)), "value_changed", G_CALLBACK(changed), this);
...
class NonModalRadio
{
	...
	public:
		NonModalRadio (const Callback& changed) :
			m_changed(changed)
		{
		}
