...
GtkScrolledWindow* create_scrolled_window (GtkPolicyType hscrollbar_policy, GtkPolicyType vscrollbar_policy,
		int border = 0);
...
inline void window_set_position (GtkWindow* window, const WindowPosition& position)
{
	...
	if (position.x < 0 || position.y < 0 || position.x > gdk_screen_get_width(screen) || position.y
...
typedef ConstReferenceCaller1<WindowPosition, const StringImportCallback&, WindowPosition_Write>
...
typedef ReferenceCaller1<WindowPositionTracker, const char*, WindowPositionTracker_importString>
...
typedef ConstReferenceCaller1<WindowPositionTracker, const StringImportCallback&, WindowPositionTracker_exportString>
