...
class MatrixTransform: public TransformNode
{
	...
	public:
		MatrixTransform () :
			m_localToParent(g_matrix4_identity)
		{
		}
...
inline Matrix4 matrix4_transform_for_components (const Translation& translation, const Rotation& rotation,
		const Scale& scale)
{
...
class TransformModifier: public Transformable
{
	...
	public:
		...
		TransformModifier (const Callback& changed, const Callback& apply) :
			m_translation(c_translation_identity), m_rotation(c_quaternion_identity), m_scale(c_scale_identity),
		...
		{
			if (m_translation != c_translation_identity || m_rotation != c_rotation_identity || m_scale
					!= c_scale_identity) {
				m_apply();
