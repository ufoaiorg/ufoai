...
ArchiveFile* ZipArchive::openFile (const std::string& name)
{
...
ArchiveTextFile* ZipArchive::openTextFile (const std::string& name)
{
	...
	if (i != m_filesystem.end() && !i->second.is_directory()) {
		...
		switch (file->m_mode) {
		case ZipRecord::eStored:
			return new StoredArchiveTextFile(name, m_name, m_istream.tell(), file->m_stream_size);
		case ZipRecord::eDeflated:
...
bool ZipArchive::read_record ()
{
	...
	if (path_is_directory(path)) {
		...
		if (!file.is_directory()) {
			globalOutputStream() << "Warning: zip archive " << m_name << " contains duplicated file: " << path << "\n";
		} else {
