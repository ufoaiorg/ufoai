...
#include "iradiant.h"   // declaration to use other interfaces as a plugin
#include "ieclass.h"

class UFOAIPluginDependencies: public GlobalRadiantModuleRef, // basic class for all other module refs
		...
		public GlobalEntityClassManagerModuleRef
{
	public:
		UFOAIPluginDependencies (void) :
			GlobalEntityModuleRef("ufo"), GlobalEntityClassManagerModuleRef("ufo")
		{
		}
};
...
namespace UFOAI
{
	...
	void dispatch (const char* command, float* vMin, float* vMax, bool bSingleBrush)
	{
		...
		if (message != NULL) {
			GlobalRadiant().m_pfnMessageBox(GTK_WIDGET(g_mainwnd), message, "Note", eMB_OK, eMB_ICONDEFAULT);
		}
		SceneChangeNotify();
	}
} // namespace
...
{
	public:
		UFOAIToolbarDependencies () :
			ModuleRef<_QERPluginTable> ("UFO:AI")
		{
		}
};
