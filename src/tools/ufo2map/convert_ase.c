...
/**
 * @brief converts a bsp drawsurface to an ase chunk
 */
...
/**
 * @brief exports a bsp model to an ase chunk
 */
static void ConvertModel (FILE *f, entity_t* e, int entity_num, vec3_t origin)
{
	int	i, s;
	...
	for ( i = 0; i < e->numbrushes; i++ ) {
		s = i + e->firstbrush;
		ConvertSurface(f, e, entity_num, s, origin);
	}
}
 ...
 * @endcode
 */
static void ConvertShader( FILE *f, char* image, int imagenum )
{
