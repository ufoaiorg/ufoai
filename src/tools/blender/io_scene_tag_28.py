...
bl_info = {
	...
	"author": "DarkRain, based on the work of Werner Hoehrer, Bob Holcomb and Damien McGinnes",
	"version": (1, 0),
	"blender": (2, 80, 0),
...
class Import_TAG(bpy.types.Operator, ImportHelper):
	...
	def execute(self, context):
		...
		for tag in range(md2Tags.numTags):
			...
			scene.collection.objects.link(blenderTag)
			blenderTag.select_set(True)
			bpy.context.view_layer.objects.active = blenderTag
...
__classes__ = (Import_TAG, Export_TAG)
