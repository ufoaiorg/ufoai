...
void H_printf(const char *fmt, ...)
{
	...
	if (!headerouthandle.f) {
		sprintf(name, "%s/tris.h", cddir);
		SafeOpenWrite (name, &headerouthandle);
		fprintf(headerouthandle.f, "/* %s */\n\n", cddir);
		fprintf(headerouthandle.f, "/* This file generated by qdata - Do NOT Modify */\n\n");
	}

	va_start(argptr, fmt);
	vfprintf(headerouthandle.f, fmt, argptr);
	va_end(argptr);
}
...
void FinishModel (void)
{
	...
	printf("saving to %s\n", name);
	CreatePath (name);
	SafeOpenWrite(name, &modelouthandle);
	if (!modelouthandle.f)
		Error("Could not load modefile: %s\n", name);
	...
	printf("%4d glcmd\n", model.num_glcmds);
	printf("%4d skins\n", model.num_skins);
	printf("file size: %d\n", (int)ftell (modelouthandle.f) );
	printf("---------------------\n");

	CloseFile(&modelouthandle);
