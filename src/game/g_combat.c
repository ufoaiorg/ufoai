	...
	if (ent->type == ET_BREAKABLE || ent->type == ET_DOOR) {
		if (!mock) {
			if (damage >= ent->HP) {
				...
				switch (ent->material) {
				case MAT_GLASS:
					gi.PositionedSound(PM_ALL, ent->origin, ent, "ambient/breakglass", CHAN_AUTO, 1, 1, 0);
					break;
				case MAT_METAL:
					gi.PositionedSound(PM_ALL, ent->origin, ent, "ambient/breakmetal", CHAN_AUTO, 1, 1, 0);
					break;
				case MAT_ELECTRICAL:
					gi.PositionedSound(PM_ALL, ent->origin, ent, "ambient/breakelectric", CHAN_AUTO, 1, 1, 0);
					break;
				case MAT_WOOD:
					gi.PositionedSound(PM_ALL, ent->origin, ent, "ambient/breakwood", CHAN_AUTO, 1, 1, 0);
					break;
				case MAT_MAX:
					break;
				}
