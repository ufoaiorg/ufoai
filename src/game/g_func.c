...
static qboolean Destroy_Breakable (edict_t *self)
{
	...
	case MAT_GLASS:
		...
		break;
	case MAT_METAL:
		gi.PositionedSound(PM_ALL, origin, self, "misc/breakmetal", 1);
		break;
	case MAT_ELECTRICAL:
		gi.PositionedSound(PM_ALL, origin, self, "misc/breakelectric", 1);
		break;
	case MAT_WOOD:
		gi.PositionedSound(PM_ALL, origin, self, "misc/breakwood", 1);
		break;
	case MAT_MAX:
		break;
	}
...
static qboolean Door_Use (edict_t *door)
{
	if (door->moveinfo.state == STATE_CLOSED) {
		...
		if (G_GameRunning()) {
			...
			if (door->noise[0] != '\0')
				gi.PositionedSound(PM_ALL, door->origin, door, door->noise, 1);
		}
