...
static void G_SpawnItemOnFloor (const pos3_t pos, const item_t *item)
{
	...
	if (floor == NULL) {
		...
		if (!game.i.TryAddToInventory(&floor->chr.i, item, INVDEF(gi.csi->idFloor))) {
			G_FreeEdict(floor);
		} else {
			...
			if (VectorLength(curV) < GRENADE_STOPSPEED || time > 4.0 || bounce > fd->bounce
			 || (!fd->delay && trEnt && G_IsActor(trEnt))) {
				...
				if (fd->splrad > 0.0) {
					...
					if (fd->ammo && !fd->splrad && weapon->item->thrown) {
						pos3_t drop;
	...
	int mask, const item_t *weapon, shot_mock_t *mock, int z_align, int i, shoot_types_t shootType, vec3_t impact)
{
	...
	if (!mock) {
		...
		if (fd->ammo && !fd->splrad && weapon->item->thrown && !weapon->item->deplete) {
			pos3_t drop;
