 ...
 */
typedef enum {
	BASE_NOT_USED, BASE_UNDER_ATTACK, /**< base is under attack */
...
typedef enum {
	BA_NONE, BA_NEWBUILDING, /**< hovering the needed base tiles for this building */
...
building_t *B_GetBuildingInBaseByType(const base_t* base, buildingType_t type,
...
qboolean B_CheckBuildingTypeStatus(const base_t* const base,
		buildingType_t type, buildingStatus_t status, int *cnt);
qboolean B_GetBuildingStatus(const base_t* const base,
		const buildingType_t type);
void B_SetBuildingStatus(base_t* const base, const buildingType_t type,
		qboolean newStatus);
...
qboolean B_UpdateStorageAndCapacity(base_t* base, const objDef_t *obj,
		int amount, qboolean reset, qboolean ignorecap);
...
void B_RemoveUFOsExceedingCapacity(base_t *base,
...
void B_SaveBaseSlotsXML(const baseWeapon_t *weapons, const int numWeapons,
		mxml_node_t *p);
