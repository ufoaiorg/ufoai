...
SWIGINTERN char const *uiNode_t_path(uiNode_t *self){ return UI_GetPath(self); }
...
static int _wrap_uiNode_path(lua_State* L) {
  int SWIG_arg = 0;
  uiNode_t *arg1 = (uiNode_t *) 0 ;
  char *result = 0 ;

  SWIG_check_num_args("uiNode_t::path",1,1)
  if(!SWIG_isptrtype(L,1)) SWIG_fail_arg("uiNode_t::path",1,"uiNode_t *");

  if (!SWIG_IsOK(SWIG_ConvertPtr(L,1,(void**)&arg1,SWIGTYPE_p_uiNode_t,0))){
    SWIG_fail_ptr("uiNode_path",1,SWIGTYPE_p_uiNode_t);
  }

  result = (char *)uiNode_t_path(arg1);
  lua_pushstring(L,(const char *)result); SWIG_arg++;
  return SWIG_arg;
