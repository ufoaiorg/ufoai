	...
	if (RS_IsResearched_ptr(od->tech)) {
		if (!Q_strncmp(od->type, "armor", 5)) {
			...
			*itemText = '\0';
			...
			Q_strcat(itemText, va(_("Action:\t%s\n"), od->fd[0][0].name), sizeof(itemText)); /* TODO: might need some changes so the correct weapon (i.e. not 0) is used for the fd */
			Q_strcat(itemText, va(_("Time units:\t%i\n"), od->fd[0][0].time), sizeof(itemText)); /* TODO: might need some changes so the correct weapon (i.e. not 0) is used for the fd */
			Q_strcat(itemText, va(_("Range:\t%g\n"), od->fd[0][0].range / 32.0), sizeof(itemText)); /* TODO: might need some changes so the correct weapon (i.e. not 0) is used for the fd */
		}

		if ( !Q_strncmp(od->type, "ammo", 4)
			 || (od->weapon && !od->reload) ) {
			Q_strcat(itemText, va(_("Primary:\t%s\t(%s)\n"), od->fd[0][0].name, CL_WeaponSkillToName(od->fd[0][0].weaponSkill)), sizeof(itemText)); /* TODO: might need some changes so the correct weapon (i.e. not 0) is used for the fd */
			Q_strcat(itemText, va(_("Secondary:\t%s\t(%s)\n"), od->fd[0][1].name, CL_WeaponSkillToName(od->fd[0][1].weaponSkill)), sizeof(itemText)); /* TODO: might need some changes so the correct weapon (i.e. not 0) is used for the fd */
			Q_strcat(itemText,
					va(_("Damage:\t%i / %i\n"), (int) ((od->fd[0][0].damage[0] + od->fd[0][0].spldmg[0]) * od->fd[0][0].shots), /* TODO: might need some changes so the correct weapon (i.e. not 0) is used for the fd */
						(int) ((od->fd[0][1].damage[0] + od->fd[0][1].spldmg[0]) * od->fd[0][1].shots)), sizeof(itemText)); /* TODO: might need some changes so the correct weapon (i.e. not 0) is used for the fd */
			Q_strcat(itemText, va(_("Time units:\t%i / %i\n"), od->fd[0][0].time, od->fd[0][1].time), sizeof(itemText)); /* TODO: might need some changes so the correct weapon (i.e. not 0) is used for the fd */
			Q_strcat(itemText, va(_("Range:\t%g / %g\n"), od->fd[0][0].range / 32.0, od->fd[0][1].range / 32.0), sizeof(itemText)); /* TODO: might need some changes so the correct weapon (i.e. not 0) is used for the fd */
			Q_strcat(itemText,
					va(_("Spreads:\t%g / %g\n"), (od->fd[0][0].spread[0] + od->fd[0][0].spread[1]) / 2, (od->fd[0][1].spread[0] + od->fd[0][1].spread[1]) / 2), sizeof(itemText)); /* TODO: might need some changes so the correct weapon (i.e. not 0) is used for the fd */
		}
