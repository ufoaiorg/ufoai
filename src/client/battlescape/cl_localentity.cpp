...
int LE_ActorGetStepTime (const le_t *le, const pos3_t pos, const pos3_t oldPos, const int dir, const int speed)
{
	if (dir != DIRECTION_FALL) {
		...
		Grid_PosToVec(cl.mapData->routing.routes, le->fieldSize, oldPos, start);
		Grid_PosToVec(cl.mapData->routing.routes, le->fieldSize, pos, dest);
...
static void LET_PathMove (le_t *le)
{
	...
	Grid_PosToVec(cl.mapData->routing.routes, le->fieldSize, le->oldPos, start);
	Grid_PosToVec(cl.mapData->routing.routes, le->fieldSize, le->pos, dest);
