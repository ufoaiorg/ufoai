...
void CL_ActorReload (le_t *le, containerIndex_t containerID)
{
	...
	} else if (containerID == csi.idLeft && inv->c[csi.idRight]->item.item->holdTwoHanded) {
...
static void CL_DumpMoveMark_f (void)
{
	...
	CL_BuildForbiddenList();
	Grid_MoveCalc(cl.mapData->map, ACTOR_GET_FIELDSIZE(selActor), &cl.pathMap, truePos, crouchingState, MAX_ROUTE_TUS, forbiddenList, forbiddenListLength);
...
static void CL_DebugPath_f (void)
{
	...
	Grid_RecalcBoxRouting(cl.mapTiles, cl.mapData->map, myBox, cl.leInlineModelList);
	}
#endif
