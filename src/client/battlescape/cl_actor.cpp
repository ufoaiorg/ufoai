...
int CL_ActorGetContainerForReload (invList_t **invList, const inventory_t *inv, const objDef_t *weapon)
{
	...
	for (container = 0; container < csi.numIDs; container++) {
		if (INVDEF(container)->out < tu) {
			 ...
			 * we've already found. */
			for (ic = inv->cont(container); ic; ic = ic->next) {
...
void CL_ActorReload (le_t *le, containerIndex_t containerID)
{
	...
	if (inv->cont(containerID)) {
		weapon = inv->cont(containerID)->item.def();
	} else if (containerID == csi.idLeft && inv->cont(csi.idRight)->item.isHeldTwoHanded()) {
...
void CL_ActorInvMove (const le_t *le, containerIndex_t fromContainer, int fromX, int fromY, containerIndex_t toContainer, int toX, int toY)
{
	...
	const invList_t *invList = INVSH_SearchInInventory(&le->inv, fromPtr, fromX, fromY);

	if (invList != NULL) {
		const character_t *chr = CL_ActorGetChr(le);
		if (!INVSH_CheckAddingItemToInventory(&le->inv, fromContainer, toContainer, invList->item, GAME_GetChrMaxLoad(chr))) {
...
//	Grid_RecalcBoxRouting(cl.mapTiles, cl.mapData->routing, myBox, cl.leInlineModelList);
	...
	GridBox mbox(cl.mapData->mapMin, cl.mapData->mapMax);
	Com_Printf("Statistics:\nWorldsize(x/y/z) %i/%i/%i\n", mbox.maxs[0] - mbox.mins[0], mbox.maxs[1] - mbox.mins[1], mbox.maxs[2] - mbox.mins[2]);
}
#endif
