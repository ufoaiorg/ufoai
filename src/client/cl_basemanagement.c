...
void B_DrawBase(menuNode_t * node)
{
		...
		for (col = 0; col < BASE_SIZE; col++) {
			...
			if (building && !secondBuilding) {
				...
				case B_STATUS_UNDER_CONSTRUCTION:
					...
					re.FontDrawString("f_small", 0, x + 10, y + 10, x + 10, y + 10, node->size[0], 0, node->texh[0], va(_("%i days left"), time), 0, 0 , NULL);
					break;
				default:
					break;
				}
	...
	if (hoverBuilding) {
		re.DrawColor(color);
		re.FontDrawString("f_small", 0, mx + 3, my, mx + 3, my, node->size[0], 0, node->texh[0], hoverBuilding->name, 0,0,NULL);
		re.DrawColor(NULL);
	}
}
