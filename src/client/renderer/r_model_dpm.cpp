...
void R_ModLoadAliasDPMModel (model_t *mod, byte *buffer, int bufSize)
{
	...
	mAliasBone_t* outBones = mod->alias.bones = Mem_PoolAllocTypeN(mAliasBone_t, mod->alias.num_bones, vid_modelPool);
	...
	mAliasMesh_t* outMesh = mod->alias.meshes = Mem_PoolAllocTypeN(mAliasMesh_t, mod->alias.num_meshes, vid_modelPool);
	for (i = 0, mesh = (dpmmesh_t *)((byte *)dpm + dpm->ofs_meshs); i < mod->alias.num_meshes; i++, mesh++, outMesh++) {
		...
		/* load skins */
		outMesh->skins = Mem_PoolAllocType(mAliasSkin_t, vid_modelPool);
		...
		mAliasBoneVertex_t* outBoneVertexes = outMesh->bonesVertexes = Mem_PoolAllocTypeN(mAliasBoneVertex_t, num, vid_modelPool);
		for (j = 0, vert = (dpmvertex_t *)((byte *)dpm + mesh->ofs_verts); j < outMesh->num_verts; j++) {
		...
		/* load texcoords */
		outMesh->stcoords = Mem_PoolAllocTypeN(mAliasCoord_t, outMesh->num_verts, vid_modelPool);
		...
		int32_t* const outIndex = outMesh->indexes = Mem_PoolAllocTypeN(int32_t, outMesh->num_tris * 3, vid_modelPool);
	...
	/* load the frames */
	mAliasFrame_t* outFrame = mod->alias.frames = Mem_PoolAllocTypeN(mAliasFrame_t, mod->alias.num_frames, vid_modelPool);
	for (i = 0, frame = (dpmframe_t *)((byte *)dpm + dpm->ofs_frames); i < mod->alias.num_frames; i++, frame++, outFrame++) {
		...
		mAliasBoneMatrix_t* outBoneMatrix = outFrame->boneMatrix = Mem_PoolAllocTypeN(mAliasBoneMatrix_t, mod->alias.num_bones, vid_modelPool);
	...
	for (i = 0; i < mod->alias.num_meshes; i++)
		R_ModLoadArrayData(&mod->alias, &mod->alias.meshes[i], true);
}
