...
public class Edge {
	...
	/** returns true if the lines of the edges meet between the vertices.
	 *               will return false if they are near-parallel. */
	public boolean intersects(Edge a){
	    if(a.dir.isParallelOrAntiparallelTo(this.dir)) return false;
	    Vector3D dirOfClosestApproach=(this.dir.cross(a.dir)).unit();
	    Vector3D fromPointOnOneLineToOther=a.point1.subtract(this.point1);
	    return Math.abs(dirOfClosestApproach.dot(fromPointOnOneLineToOther)) < Epsilon.distance;
	}
	
	/** see http://mathworld.wolfram.com/Line-LineIntersection.html
	 * @return the point of intersection.
	 *  @throw NotInLineWithEdgeException if there is no intersection or the lines 
	 *          of the edges intersect outside the bounds of the vertices. */
	public Vector3D intersection(Edge a) throws NotInLineWithEdgeException{
	    if(!intersects(a)) throw new NotInLineWithEdgeException();
	    Vector3D av=this.dir;
	    Vector3D bv=a.dir;
	    Vector3D cv=a.point1.subtract(this.point1);
	    Vector3D cvxbv=cv.cross(bv);
	    Vector3D avxbv=av.cross(bv);
	    float magavxbv=avxbv.magnitude();
	    float paramthis=(cvxbv).dot(avxbv)/(magavxbv*magavxbv);
	    Vector3D lineIntersection = this.point1.add(av.multiply(paramthis));
	    if(this.isBetweenVertices(lineIntersection) && a.isBetweenVertices(lineIntersection)) {
		return lineIntersection;
	    } else {
		throw new NotInLineWithEdgeException();
	    }
	}
	...
	/** @return true if the supplied point is between the vertices of this edge
	 *  @throw NotInLineWithEdgeException if the point is not in line with the edge
	 */
	public boolean isBetweenVertices(Vector3D p) throws NotInLineWithEdgeException {
	    return isDistanceOnEdge(distanceAlongEdge(p))==0;
	}
	...
	public static void main (String[] args) {
	    ...
	    System.out.println("");
	    Vector3D L1P1=new Vector3D(0f,0f,0f);
	    Vector3D L1P2=new Vector3D(1f,1f,1f);
	    Edge E1=new Edge(L1P1,L1P2);
	    Vector3D L2P1=new Vector3D(1f,1f,2f);
	    Vector3D L2P2=new Vector3D(1f,1f,0f);
	    Edge E2=new Edge(L2P1,L2P2);
	    System.out.println("trying intersection");
	    try{
		System.out.println("intersection:"+E1.intersection(E2));
	    } catch (NotInLineWithEdgeException eNILWE){System.out.println("NotInLineWithEdgeException thrown");}
	}
